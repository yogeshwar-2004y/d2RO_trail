  ...
  methods: {
    ...
    downloadDashboardPDF() {
      try {
        ...
        // Header - Reports Dashboard Summary
        doc.setFontSize(20);
        doc.setFont("helvetica", "bold");
        doc.text("REPORTS DASHBOARD SUMMARY", pageWidth / 2, yPosition, {
          align: "center",
        });
        yPosition += 15;
        ...
        doc.text(`Generated on: ${currentDate} at ${currentTime}`, pageWidth / 2, yPosition, {
          align: "center",
        });
        yPosition += 10;
        ...
        if (currentUser) {
          doc.setFontSize(10);
          doc.text(`Generated by: ${currentUser.name} (${currentUser.role})`, pageWidth / 2, yPosition, {
            align: "center",
          });
          yPosition += 15;
        }
        ...
        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.text(`Total Reports: ${this.filteredReports.length}`, margin, yPosition);
        yPosition += 6;
        ...
        this.filteredReports.forEach((report, index) => {
          ...
          doc.text(report.project || 'N/A', tableStartX + snoWidth, yPosition);
          doc.text(
            report.name || 'N/A',
            tableStartX + snoWidth + projectWidth,
            yPosition
        ...
        doc.text("This report was generated from the AVIATRAX Reports Dashboard", pageWidth / 2, yPosition, {
          align: "center",
        });

        // Save the PDF
        const fileName = `Reports_Dashboard_Summary_${currentDate.replace(
          /\//g,
          "-"
        )}.pdf`;
        doc.save(fileName);
