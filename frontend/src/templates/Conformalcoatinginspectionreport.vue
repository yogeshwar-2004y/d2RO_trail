<template>
  <div class="conformal-coating-inspection-page">
    ...
    <div class="report-info">
      <div><strong>Project Name:</strong> <input v-model="projectName" type="text" /></div>
      <div><strong>Report Ref No:</strong> <input v-model="reportRefNo" type="text" /></div>
      <div><strong>DP Name:</strong> <input v-model="dpName" type="text" /></div>
      <div><strong>Part No:</strong> <input v-model="partNo" type="text" /></div>
      <div><strong>SL No’s:</strong> <input v-model="slNo" type="text" /></div>
      <div><strong>End Date:</strong> <input v-model="endDate" type="date" /></div>
      <div><strong>SRU Name:</strong> <input v-model="sruName" type="text" /></div>
      <div><strong>Start Date:</strong> <input v-model="startDate" type="date" /></div>
      <div><strong>Inspection Stage:</strong> <input v-model="inspectionStage" type="text" /></div>
      <div><strong>Test Venue:</strong> <input v-model="testVenue" type="text" /></div>
    </div>
    ...
    <table class="inspection-table">
      ...
      <tbody>
        ...
        <div class="report-footer">
          <div><strong>Prepared By:</strong> <input v-model="preparedBy" type="text" /></div>
          <div><strong>Verified By:</strong> <input v-model="verifiedBy" type="text" /></div>
          <div><strong>Approved By:</strong> <input v-model="approvedBy" type="text" /></div>
        </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      tests: [
        { case: "Uncoated patches", expected: "Should not be there", observation: "", remark: "" },
        { case: "Entrapped air bubbles", expected: "Should not be there", observation: "", remark: "" },
        { case: "Enclosed foreign particles in the coat", expected: "Should not be there", observation: "", remark: "" },
        { case: "Residue of masking material", expected: "Should not be there", observation: "", remark: "" },
        { case: "Discoloration", expected: "No recommended", observation: "", remark: "" },
        { case: "Cracks", expected: "No Crimony", observation: "", remark: "" },
        { case: "Pin holes and Soft spots", expected: "No Damages", observation: "", remark: "" },
        { case: "Connectors surrounding and beneath", expected: "Should have linear coating", observation: "", remark: "" },
        { case: "Conformal coating thickness of 30 to 130 microns for acrylic material as per IPC-CC-830B", expected: "30 to 130 microns", observation: "", remark: "" }
      ]
    };
  },
  computed: {
  ...
  methods: {
    ...
    exportReport() {
      try {
        ...
        doc.text('CONFORMAL COATING INSPECTION REPORT', pageWidth / 2, yPosition, { align: 'center' });
        yPosition += 15;
        ...
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        const subjectText = `SUB: Conformal Coating Inspection Report for ${this.lruName || 'Unknown LRU'}`;
        ...
        if (this.tests && this.tests.length > 0) {
          ...
          this.tests.forEach((test, index) => {
            ...
            doc.text(test.observation.substring(0, 20), margin + 120, yPosition);
        ...
        doc.setFont('helvetica', 'normal');
        doc.text(`Prepared By: ${this.preparedBy || 'Not specified'}`, margin, yPosition);
        yPosition += 6;
        doc.text(`Verified By: ${this.verifiedBy || 'Not specified'}`, margin, yPosition);
        yPosition += 6;
        doc.text(`Approved By: ${this.approvedBy || 'Not specified'}`, margin, yPosition);
        
        // Save PDF
        const fileName = `Conformal_Coating_Inspection_Report_${this.lruName || 'Unknown'}_${this.currentDate.replace(/\//g, '-')}.pdf`;
        doc.save(fileName);
