...
export default {
  ...
  methods: {
    ...
    async uploadBackground() {
      ...
      try {
        const response = await fetch('http://127.0.0.1:8000/api/upload-login-background', {
          method: 'POST',
          body: formData
        });
    ...
    async resetToDefault() {
      try {
        const response = await fetch('http://127.0.0.1:8000/api/reset-login-background', {
          method: 'POST'
        });
  ...
  async mounted() {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/get-current-background');
      const data = await response.json();
