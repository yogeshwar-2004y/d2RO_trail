  ...
  <div class="memo-dashboard">
    <div class="header">
      ...
      <div class="header-right">
        ...
        <!-- NEW: Submit New Memo Button (Conditional) -->
        <button v-if="canSubmitMemo" class="submit-new-memo-button" @click="submitNewMemo">
          + Submit New Memo
        </button>
...
export default {
  ...
  data() {
    return {
      ...
      // This will eventually come from your authentication system (e.g., Vuex store, localStorage)
       currentUserRole: 'Design Head', // Placeholder: 'Design Head', 'Design Member', 'Admin', etc.
  ...
  computed: {
    ...
    canSubmitMemo() {
      // Determine if the current user role can submit a new memo
      return ['Design Head', 'Design Member'].includes(this.currentUserRole);
    }
  },
  methods: {
    toggleProjectFilter() {
    ...
    submitNewMemo() {
      // Navigate to the memo submission form
...
.submit-new-memo-button {
  background: linear-gradient(to right, #a0d9e6, #5cb8de); /* A pleasant blue gradient */
  color: white;
  border: none;
  border-radius: 20px;
  padding: 10px 15px;
