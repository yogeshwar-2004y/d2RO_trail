...
def create_conformal_coating_inspection_report():
    ...
    try:
        ...
        try:
            ...
            print(f"    original_report_id will be added separately: {original_report_id}")
            ...
            try:
                # Check if original_report_id column exists, if not add it
                cur.execute("""
                    SELECT column_name 
                ...
                """)
                if not cur.fetchone():
                    print("  Adding original_report_id column to conformal_coating_inspection_report table...")
                    cur.execute("""
                        ALTER TABLE conformal_coating_inspection_report 
                        ADD COLUMN original_report_id INT REFERENCES reports(report_id) ON DELETE CASCADE
                    """)
                    conn.commit()
                    print("  ✓ Added original_report_id column")
                ...
                """, insert_values + (original_report_id,))
...
def get_conformal_coating_inspection_report(report_id):
    ...
    try:
        ...
        # First try to find by original_report_id (from reports table)
        cur.execute("""
...
def notify_qa_heads_approval_conformal_coating():
    """Notify QA Heads when a conformal coating inspection report is approved"""
    try:
        data = request.json
        if not data:
            return jsonify({"success": False, "message": "No data provided"}), 400
        ...
        # Log activity to activity logs
        from utils.activity_logger import log_activity
        activity_message = f"Conformal Coating Inspection Report (ID: {report_id}, Ref: {report_ref_no}) has been approved by {approver_name} for project '{project_name}', LRU '{lru_name}'."
        
        activity_logged = log_activity(
            project_id=None,  # Report operations don't have project_id in this context
            activity_performed="Conformal Coating Inspection Report Approved",
        ...
        if activity_logged:
            print(f"✓ Activity logged: Conformal Coating Inspection Report {report_id} approved by {approver_name}")
        else:
